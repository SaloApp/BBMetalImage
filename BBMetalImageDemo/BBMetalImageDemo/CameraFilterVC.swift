...
class CameraFilterVC: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    metalView = BBMetalView(
      frame: CGRect(x: x, y: 100, width: width, height: view.bounds.height - 230))
  ...
  private var filter: BBMetalBaseFilter? {
    ...
    case .highlightShadowTint:
      return BBMetalHighlightShadowTintFilter(
        shadowTintColor: .blue,
    ...
    case .monochrome:
      return BBMetalMonochromeFilter(
        color: BBMetalColor(red: 0.7, green: 0.6, blue: 0.5), intensity: 1)
    ...
    case .chromaKey:
      return BBMetalChromaKeyFilter(
        thresholdSensitivity: 0.4, smoothing: 0.1, colorToReplace: .blue)
    case .crop:
      return BBMetalCropFilter(rect: BBMetalRect(x: 0.25, y: 0.5, width: 0.5, height: 0.5))
    ...
    case .zoomBlur:
      return BBMetalZoomBlurFilter(blurSize: 3, blurCenter: BBMetalPosition(x: 0.35, y: 0.55))
    ...
    case .chromaKeyBlend:
      imageSource = BBMetalStaticImageSource(image: topBlendImage(withAlpha: 0.1))
      return BBMetalChromaKeyBlendFilter(
        thresholdSensitivity: 0.4, smoothing: 0.1, colorToReplace: .blue)
    case .dissolveBlend:
      imageSource = BBMetalStaticImageSource(image: topBlendImage(withAlpha: 0.1))
    ...
    case .polarPixellate:
      return BBMetalPolarPixellateFilter(
        pixelSize: BBMetalSize(width: 0.05, height: 0.03), center: BBMetalPosition(x: 0.35, y: 0.55)
      )
    ...
    case .swirl:
      return BBMetalSwirlFilter(center: BBMetalPosition(x: 0.35, y: 0.55), radius: 0.25, angle: 1)
    ...
    case .convolution3x3:
      return BBMetalConvolution3x3Filter(
        convolution: simd_float3x3(rows: [
          SIMD3<Float>(-1, 0, 1),
          SIMD3<Float>(-2, 0, 2),
          SIMD3<Float>(-1, 0, 1),
        ]))
