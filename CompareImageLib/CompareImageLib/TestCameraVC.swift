...
class TestCameraVC: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    case .BBMetalImage:
      do {
        bbCamera = try BBMetalCamera(sessionPreset: .high)
      } catch {
        print("Failed to create BBMetalCamera: \(error)")
        return
      }
      bbCamera.benchmark = true

      bbLookupFilter = BBMetalLookupFilter(
        lookupTable: UIImage(named: "test_lookup")!.bb_metalTexture!)

      let contrastFilter = BBMetalContrastFilter(contrast: 3)
      let lookupFilter = BBMetalLookupFilter(
        lookupTable: UIImage(named: "test_lookup")!.bb_metalTexture!)
      ...
      bbImageView = BBMetalView(
        frame: CGRect(
          x: 10, y: 100, width: view.bounds.width - 20, height: view.bounds.height - 200))
      view.addSubview(bbImageView)

      bbCamera.add(consumer: bbImageView)

    case .GPUImage:
      gpuCamera = GPUImageVideoCamera(
        sessionPreset: AVCaptureSession.Preset.high.rawValue, cameraPosition: .back)
    ...
    let segment = UISegmentedControl(
      frame: CGRect(x: 10, y: view.bounds.height - 90, width: view.bounds.width - 20, height: 30))
    ...
    let benchmarkButton = UIButton(
      frame: CGRect(x: 10, y: view.bounds.height - 60, width: view.bounds.width - 20, height: 30))
    benchmarkButton.backgroundColor = .green
    benchmarkButton.setTitle("Benchmark", for: .normal)
    benchmarkButton.addTarget(
      self, action: #selector(clickBenchmarkButton(_:)), for: .touchUpInside)
